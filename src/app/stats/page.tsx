...
export default function DashboardStats() {
  ...
  const previousMonth =
    mockData.monthlyComparison[mockData.monthlyComparison.length - 2];
  const monthlyGrowth =
    ((currentMonth.memberships - previousMonth.memberships) /
      previousMonth.memberships) *
    100;

  const totalMemberships = mockData.membershipsByType.reduce(
    (sum, m) => sum + m.count,
    0
  );
            ...
            <div className="pt-2 border-t border-primary200">
              <p className="text-sm text-white/70">
                ...
                <span className="font-medium">
                  {totalMemberships} membresías activas
                </span>
              </p>
            </div>
          </CardContent>
            ...
            <div className="space-y-3">
              {mockData.monthlyComparison.map((month, index) => {
                const isCurrentMonth =
                  index === mockData.monthlyComparison.length - 1;
        ...
        <Card className="border-2 border-dashed border-gray-300">
          <CardContent className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm text-gray-500 max-w-sm">
              Espacio reservado para futuras métricas como ingresos, retención
              de clientes, horarios pico, etc.
            </p>
          </CardContent>
        </Card>
