-- =============================================================================
-- Migration: upsert_customer_membership_with_payment
--            (decouple type change from p_is_paid)
-- =============================================================================
-- La versión anterior (20260707113341) ataba todo el flujo de UPDATE del
-- pago vigente al checkbox "Pagar cuota" del form (p_is_paid = true). Eso
-- era incorrecto: el operador que cambia el tipo intra-vigencia no está
-- cobrando (el cliente ya pagó), pero sí necesita que el pago vigente
-- refleje el nuevo tipo para no dejar el registro contable inconsistente
-- con customer_membership.
--
-- Cambios:
--   * El UPDATE del pago vigente ahora se dispara por dos motivos
--     independientes:
--       (a) p_is_paid = true → cobro/corrección desde el form (amount y
--           método vienen del form).
--       (b) Cambio de tipo intra-vigencia → aunque p_is_paid = false,
--           actualiza membership_type y amount del pago vigente. El amount
--           se toma del types_memberships.amount del tipo nuevo.
--           payment_date y payment_method se preservan (no hay cobro
--           nuevo).
--   * refund/charge_diff siguen disparándose por p_type_change_action,
--     independiente de p_is_paid.
--
...
BEGIN
  ...
  -- Bloquear la fila del cliente para el resto de la transacción.
  ...
  -- ==========================================================================
  -- Rama 1: cobro/corrección con p_is_paid = true
  -- ==========================================================================
  ...
  -- ==========================================================================
  -- Rama 2: cambio de tipo intra-vigencia sin cobro (p_is_paid = false)
  --         Actualiza tipo y amount del pago vigente para mantener
  --         coherencia contable; preserva payment_date y payment_method.
  -- ==========================================================================
  ...
  -- ==========================================================================
  -- Ajustes contables por cambio de tipo (independiente de p_is_paid).
  -- Solo si el operador tildeó el checkbox correspondiente en el form.
  -- ==========================================================================
  ...
  -- Asistencia (para cualquier tipo, incluida VIP).
