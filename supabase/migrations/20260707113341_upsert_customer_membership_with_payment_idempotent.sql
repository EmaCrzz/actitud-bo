...
--            (idempotencia por vigencia + soporte de refund/charge_diff +
--             restricción de VIP a admins)
-- =============================================================================
-- Cambios respecto a la versión anterior (20260701170000_argentina_tz):
--
-- 1) Idempotencia: si la membresía sigue vigente y el tipo no cambia, se
--    UPDATEA la fila apuntada por customer_membership.current_payment_id
--    en lugar de INSERTAR una nueva. Esto elimina los duplicados que
--    corrompían la contabilidad.
--
-- 2) Ajustes por cambio de tipo intra-vigencia:
--    - p_type_change_action = 'refund':   UPDATE del pago + INSERT en
--      expenses (categoría 'reintegros') por la diferencia.
--    - p_type_change_action = 'charge_diff': UPDATE del pago + INSERT
--      nueva fila en membership_payments con el delta a cobrar.
--    - p_adjustment_amount es el monto del ajuste (editable por el
--      operador desde el form).
--
-- 3) VIP solo puede ser asignada por usuarios con rol 'admin'. Defensa en
--    profundidad: aunque el UI filtre VIP para no-admins, cualquier
--    request directa a la RPC se rechaza.
--
-- 4) DAILY sigue haciendo INSERT (vence el mismo día).
--
-- 5) Concurrencia: SELECT ... FOR UPDATE sobre customer_membership al
--    inicio para evitar que dos operadores dupliquen el pago del mismo
...
BEGIN
  ...
  RAISE NOTICE 'State: is_active=%, same_type=%, is_daily=%, is_vip=%, current_payment_id=%',
    v_is_active, v_same_type, v_is_daily, v_is_vip, v_current.current_payment_id;
