-- =============================================================================
-- Migration: consolida payment_method en membership_payments a un único
--            vocabulario canónico ('PAYMENT_CASH', 'PAYMENT_TRANSFER'),
--            agrega CHECK constraint, y hace fail-loud en el RPC cuando
--            p_payment_type viene vacío en flujos que escriben pagos.
-- =============================================================================
-- Contexto:
--   La columna membership_payments.payment_method quedó con valores
--   heterogéneos por convivencia de dos sistemas de tipos en el código
--   ('PAYMENT_CHASH'/'PAYMENT_TRANSFER' del sistema activo y
--    'efectivo'/'transferencia'/'Efectivo' del sistema muerto en
--    src/accounting/consts.ts). El RPC además tenía un fallback silencioso
--    COALESCE(p_payment_type, 'efectivo') que enmascaraba faltantes reales
--    del formulario en el flujo charge_diff.
--
-- Cambios:
--   1) Backfill de valores existentes al vocabulario canónico,
--      corrigiendo también el typo 'CHASH' → 'CASH'.
--   2) CHECK constraint para bloquear futuros valores fuera del vocabulario.
--   3) CREATE OR REPLACE del RPC reemplazando las tres apariciones de
--      COALESCE(p_payment_type, 'efectivo') por RAISE EXCEPTION cuando
--      el flujo va a escribir en membership_payments sin payment_type.
-- =============================================================================

-- 1) Backfill: normalizar todos los valores existentes al vocabulario canónico.
UPDATE public.membership_payments
SET payment_method = 'PAYMENT_CASH'
WHERE payment_method IN ('PAYMENT_CHASH', 'Efectivo', 'efectivo');

UPDATE public.membership_payments
SET payment_method = 'PAYMENT_TRANSFER'
WHERE payment_method IN ('Transferencia', 'transferencia');

-- 2) CHECK constraint: blindar la columna para futuros inserts/updates.
...
-- 3) RPC: fail-loud cuando p_payment_type es NULL y hay escritura en pagos.
