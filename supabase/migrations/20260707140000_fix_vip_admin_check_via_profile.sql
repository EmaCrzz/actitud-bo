-- =============================================================================
-- Migration: fix chequeo de admin para VIP en el RPC
-- =============================================================================
-- La versión anterior del chequeo hacía:
--   WHERE user_roles.user_id = auth.uid()
-- pero user_roles.user_id apunta a profile.id, no a auth.users.id.
-- El vínculo real es profile.auth_id = auth.uid() → profile.id → user_roles.user_id.
--
-- Sin este join, cualquier admin era rechazado con UNAUTHORIZED_VIP_ASSIGNMENT
-- porque auth.uid() nunca matcheaba con user_roles.user_id.
--
-- Este mismo patrón lo usan getCurrentUserRoles (server) y useAuth (client).
-- =============================================================================
...
BEGIN
  ...
  -- Restricción: solo admins pueden asignar VIP. Chequeo con el join real:
  -- auth.uid() = profile.auth_id, y user_roles.user_id = profile.id.
